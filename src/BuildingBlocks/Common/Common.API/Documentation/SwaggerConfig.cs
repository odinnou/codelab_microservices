...
namespace Common.API.Documentation
{
    public static class SwaggerConfig
    {
        public static IServiceCollection AddSwagger(this IServiceCollection services, string deployedVersion)
        {
            services.AddSwaggerGen(c =>
            {
                c.SwaggerDoc("api", new OpenApiInfo
                {
                    ...
                    Title = $"Formation Microservices {Assembly.GetEntryAssembly()!.GetName().Name}",
                    Description = $"Formation Microservices {Assembly.GetEntryAssembly()!.GetName().Name}, version : {deployedVersion}"
                });
        ...
        public static IApplicationBuilder UseSwaggerConfig(this IApplicationBuilder app, string deployedVersion, string routePrefix, string httpRequestScheme)
        {
            ...
            app.UseSwaggerUI(c =>
            {
                c.SwaggerEndpoint($"./swagger/api/swagger.json", $"Formation Microservices {Assembly.GetEntryAssembly()!.GetName().Name} {deployedVersion}");
